// Object paths for "Hosted by UNI·SIM" export agreements — and the repair for
// the ones the old flow filed under a name that was never real.
//
...
//   2. upload the bytes to `<org>/exports/<upload_id>-<slug>.pdf`
...
// EVERY hosted agreement's ledger row kept `storage_path = 'pending'`. The list
// in the Back up dialog reads the ledger, so the backup shows up; opening it
...
//
//   * `hostedExportPath` names the object BEFORE the token is reserved, so the
//     ledger records the truth at insert time and step 3 disappears; and
//   * `hostedExportPathCandidates` rebuilds where a legacy row's bytes actually
//     went. The old path was fully determined by data still on the row —
//     `<org_id>/exports/<id>-<safeName(file_name)>` — so a 'pending' row is
...
/** The `exports` segment of every Universal Exports object path. */
export const HOSTED_PRODUCT = "exports";

/** The placeholder the old three-step flow filed rows under. */
export const PENDING_PATH = "pending";

/**
 * Slug + `.pdf` for the object name. Byte-for-byte the `safeName` the store
...
export function safeName(fileName: string | null | undefined): string {
  const base = (fileName ?? "export-agreement").replace(/\.pdf$/i, "");
...
/**
 * Where a hosted agreement lives:
 * `hosted-uploads/<org_id>/exports/<object_id>-<slug>.pdf`.
 *
 ...
 */
export function hostedExportPath(orgId: string, objectId: string, fileName: string | null | undefined): string {
 ...
 */
export function hostedExportPathCandidates(upload: HostedUploadRef): string[] {
  const out: string[] = [];
  const recorded = upload.storage_path?.trim();
  if (isUsableStoragePath(recorded, upload.org_id) && recorded) out.push(recorded);
  const legacy = hostedExportPath(upload.org_id, upload.id, upload.file_name);
