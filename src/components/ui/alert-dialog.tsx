...
>(({ className, ...props }, ref) => (
  <AlertDialogPrimitive.Overlay
    className={cn(
      // ⚠️ z-[1100] — see the note in dialog.tsx. UniversalAppsNavBar's inline
      // `zIndex: 1000` beats every Tailwind z class.
...
>(({ className, ...props }, ref) => (
  <AlertDialogPortal>
    ...
    <AlertDialogPrimitive.Content
      ref={ref}
      className={cn(
        // Capped at the viewport and laid out as a column: the title and the
        // action row stay put, and only the description scrolls (see
        // AlertDialogHeader / AlertDialogDescription below). An alert dialog
        // has no body slot of its own, so the split lives in the primitives
...
const AlertDialogHeader = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn("flex min-h-0 flex-1 flex-col space-y-2 text-center sm:text-left", className)} {...props} />
);
...
>(({ className, ...props }, ref) => (
  <AlertDialogPrimitive.Title ref={ref} className={cn("shrink-0 text-lg font-semibold", className)} {...props} />
));
...
>(({ className, ...props }, ref) => (
  <AlertDialogPrimitive.Description
    ref={ref}
    className={cn("-mx-6 min-h-0 flex-1 overflow-y-auto px-6 text-sm text-muted-foreground", className)}
    {...props}
  />
));
